...
def show_search_form():
    ...
    search_form = App("Search Form", width=400, height=300)

    # Create a box to hold the form components
    form_content = Box(search_form, align="top")

    # Create form labels and input field
    Text(form_content, text="Last Name:")
    last_name_input = TextBox(form_content)

    # Create a box to display search results
    search_results_box = Box(search_form, align="top")

    # Function to perform member search
    def perform_search():
        search_term = last_name_input.value

        # Perform search logic here
        # Replace the following example code with your own implementation
        ...
        if search_term:
            ...
            results = [
                {"name": "John Doe", "membership_type": "Premium"},
                {"name": "Jane Smith", "membership_type": "Basic"},
                {"name": "David Johnson", "membership_type": "VIP"}
            ]

            # Display search results
            for result in results:
                Text(search_results_box, text=f"Name: {result['name']}\nMembership Type: {result['membership_type']}", size=12, font="Arial", color="black")

        else:
            info("Search Form", "Please enter a last name.")

    # Create a button to perform the search
    search_button = PushButton(form_content, text="Search", command=perform_search)
...
# Function to show fitness form
def show_fitness_form():
    # Create a new window for the fitness form
    fitness_form = App("Fitness Form", width=400, height=300)

    # Create a box to hold the form components
    form_content = Box(fitness_form, align="top")

    # Create form labels and input field
    Text(form_content, text="Select Class:")
    class_input = Combo(form_content, options=["Cardio, Thursday, 3pm-5pm", "Pilates, Friday, 9am-11am", "Spin, Monday, 2pm-4pm"])

    # Function to enroll in a fitness class
    def enroll_in_class():
        selected_class = class_input.value

        # Perform enrollment logic here
        # Replace the following example code with your own implementation

        # Example enrollment logic
        info("Fitness Form", f"You have successfully enrolled in {selected_class}!")
        fitness_form.destroy()

    # Create a button to enroll in the class
    enroll_button = PushButton(form_content, text="Enroll", command=enroll_in_class)

    # Start the fitness form
    fitness_form.display()
...
def show_help_screen():
    info("Help Screen", "This is the help screen.")
    # Create a new window for the help screen
    help_screen = App("Help Screen", width=400, height=300)

    # Create a box to hold the help content
    help_content = Box(help_screen, align="top")

    # Add static text with help information
    Text(help_content, text="Welcome to the Gym Membership Application Help Screen", size=14, font="Arial", color="black")
    Text(help_content, text="This application allows you to perform the following actions:", size=12, font="Arial", color="black")
    Text(help_content, text="- Fill out a membership form", size=12, font="Arial", color="black")
    Text(help_content, text="- Search for members by last name", size=12, font="Arial", color="black")
    Text(help_content, text="- Enroll in fitness classes", size=12, font="Arial", color="black")
    Text(help_content, text="To navigate between screens, use the buttons at the top.", size=12, font="Arial", color="black")
    Text(help_content, text="If you need further assistance, please contact the support team.", size=12, font="Arial", color="black")
...
# Create the main app
app = App("Gym Membership Application")

# Create a box to hold the navigation components
navigation_box = Box(app, align="top")

# Create navigation buttons
membership_btn = PushButton(navigation_box, text="Membership Form", command=show_membership_form)
search_btn = PushButton(navigation_box, text="Search Form", command=show_search_form)
fitness_btn = PushButton(navigation_box, text="Fitness Form", command=show_fitness_form)
help_btn = PushButton(navigation_box, text="Help", command=show_help_screen)

# Create the main screen content
main_screen_content = Box(app, align="top")
Text(main_screen_content, text="Welcome to the Gym Membership Application!", size=16, font="Arial", color="blue")
